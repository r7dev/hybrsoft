...
{
	public class StudentBelongingEmbeddingService(IDataServiceFactory dataServiceFactory,
		IEmbeddingService embeddingService) : IEmbeddingTables
	{
		...
		public async Task PopulateMissingEmbeddingsAsync()
		{
			...
			var items = await dataService.GetStudentBelongingWithMissingEmbeddingsAsync();
			...
			for (int i = 0; i < items.Count; i += batchSize)
			{
				...
				var newEmbeddings = new List<StudentBelongingEmbedding>();

				foreach (var item in batchItems)
				{
					terms.Add(item.SearchTerms);

					var embedding = item.StudentBelongingEmbeddings?.FirstOrDefault()
						?? new StudentBelongingEmbedding { StudentBelongingID = item.StudentBelongingID, SearchTerms = item.SearchTerms };
				...
				await dataService.UpdateStudentBelongingEmbeddingsAsync([.. newEmbeddings]);
			}
		}
	}
}
